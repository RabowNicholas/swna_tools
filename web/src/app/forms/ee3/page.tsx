...
export default function EE3Form() {
  ...
  const onSubmit = async (data: EE3FormData) => {
    ...
    try {
      ...
      const formattedEmploymentHistory = data.employment_history.map(job => ({
        ...job,
        start_date: job.start_date ? new Date(job.start_date).toISOString().split('T')[0] : null,
        end_date: job.end_date ? new Date(job.end_date).toISOString().split('T')[0] : null,
      }));
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `EE3_${data.first_name.charAt(0) || 'X'}.${data.last_name}_${new Date().toLocaleDateString('en-US').replace(/\//g, '.')}.pdf`;
  ...
  const handlePortalAccess = async () => {
    ...
    try {
      const selectedClient = clients.find(c => c.id === lastFormData.client_id);
      if (!selectedClient) {
        throw new Error('Selected client not found');
      }
      ...
      const ssnLast4 = clientName.split('-').pop()?.trim() || lastFormData.ssn.slice(-4);
      ...
      if (response.ok) {
        alert('Portal automation completed! Please select and upload your PDF file manually.');
      } else {
  ...
  const employmentRequiredFields = watchedFields.employment_history?.length * 7 || 0; // 7 required fields per employment
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Generate EE-3 Form</h1>
            <p className="text-muted-foreground">Employee employment history documentation with automated portal submission</p>
          </div>
          <Badge variant={progressPercentage === 100 ? 'success' : 'secondary'} size="lg">
        ...
        <Card variant="elevated">
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <Badge variant="outline">{fields.length} Job{fields.length !== 1 ? 's' : ''}</Badge>
              </div>
              <div className="flex items-center space-x-3">
                ...
                {fields.length > 1 && (
                  <div className="flex items-center space-x-2">
                    ...
                    <Button
                      type="button"
                      onClick={() => setCollapsedEmployment(new Set(Array.from({ length: fields.length }, (_, i) => i)))}
                      variant="ghost"
                      size="sm"
                      className="text-muted-foreground hover:text-foreground"
                    >
          ...
          <CardContent>
            ...
            {fields.map((field, index) => {
              ...
              return (
                <Card key={field.id} variant="outlined" className={`mb-6 last:mb-0 transition-all duration-200 ${
                          ...
                          {previewData && (
                            <div className="text-sm text-muted-foreground mt-1 space-y-1">
                              <div className="flex items-center space-x-4">
                                <span className="font-medium">{employment?.facility_name}</span>
                                <span>{employment?.position_title}</span>
                              </div>
                              <div className="text-xs">
                                {employment?.city}, {employment?.state} • {employment?.start_date} - {employment?.end_date || 'Present'}
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center space-x-2">
                        {isComplete && (
                          <Button
                            ...
                            variant="ghost"
                            size="sm"
                            icon={showPreview[index] ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                            className="text-muted-foreground hover:text-foreground"
                          >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Input
                      label="Facility Name"
                      required
                      error={form.formState.errors.employment_history?.[index]?.facility_name?.message}
                      {...form.register(`employment_history.${index}.facility_name`)}
                    />

                    <Input
                      label="Specific Location"
                      required
                      error={form.formState.errors.employment_history?.[index]?.specific_location?.message}
                      {...form.register(`employment_history.${index}.specific_location`)}
                    />

                    <Input
                      label="City"
                      required
                      error={form.formState.errors.employment_history?.[index]?.city?.message}
                    ...
                    <Input
                      ...
                      error={form.formState.errors.employment_history?.[index]?.state?.message}
                    ...
                    <Input
                      label="Contractor"
                      required
                      error={form.formState.errors.employment_history?.[index]?.contractor?.message}
                      {...form.register(`employment_history.${index}.contractor`)}
                    />

                    <Input
                      label="Position Title"
                      required
                      error={form.formState.errors.employment_history?.[index]?.position_title?.message}
                      {...form.register(`employment_history.${index}.position_title`)}
                    />
                    ...
                    <div className="md:col-span-2">
                      <Textarea
                        ...
                        error={form.formState.errors.employment_history?.[index]?.work_duties?.message}
                        {...form.register(`employment_history.${index}.work_duties`)}
                      />
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        <label className="flex items-center space-x-2 p-3 border border-border rounded-lg hover:bg-accent cursor-pointer">
                          <input
                            type="checkbox"
                            {...form.register(`employment_history.${index}.union_member`)}
                        ...
                        <label className="flex items-center space-x-2 p-3 border border-border rounded-lg hover:bg-accent cursor-pointer">
                          <input
                            type="checkbox"
                            {...form.register(`employment_history.${index}.dosimetry_worn`)}
                            className="rounded border-border text-primary focus:ring-ring"
                          />
                          <span className="text-sm font-medium">Dosimetry Badge Worn</span>
                        </label>
                      </div>
                    </div>
                  </div>
