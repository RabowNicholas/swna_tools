...
export default function DolLetterForm() {
  ...
  const onSubmit = async (data: DolLetterFormData) => {
    setLoading(true);
    try {
      ...
      const response = await fetch("/api/generate/dol-letter", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const currentDate = new Date()
          ...
          .replace(/\//g, ".");
        a.download = `DOL_Letter_${nameForFile}_${currentDate}.pdf`;
      ...
      console.error("Error generating DOL letter:", error);
      alert(
        error instanceof Error
          ? error.message
          : "Failed to generate DOL letter"
      );
    } finally {
      setLoading(false);
    }
          ...
          <>
            <Card variant="elevated">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Client information and letter date
                </p>
              </CardHeader>
              <CardContent>
            ...
            <Card variant="elevated">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Type your letter below. The closing sentence will be added automatically.
                </p>
              </CardHeader>
              <CardContent>
                <Textarea
          ...
          <CardContent className="p-8">
            <div className="text-center space-y-6">
              <div className="flex justify-center">
                <Button
                  type="submit"
                  disabled={loading || !form.watch("client_id") || !form.watch("letter_content")}
                  variant="primary"
                  hierarchy="primary"
                  size="xl"
                  loading={loading}
              ...
              {!form.watch("client_id") && (
                <p className="text-sm text-muted-foreground">
                  Select a client above to generate a custom letter
                </p>
              )}
            </div>
          </CardContent>
          ...
          <Card variant="elevated" className="bg-success/10 border-success/20">
            <CardContent className="p-6">
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h3 className="text-lg font-medium text-foreground mb-2">
                    Custom DOL letter generated successfully!
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    Your custom DOL letter has been generated and downloaded. You can now upload it to the DOL portal or save it for your records.
                  </p>
                </div>
              </div>
            </CardContent>
