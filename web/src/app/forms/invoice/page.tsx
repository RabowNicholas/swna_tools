...
export default function InvoiceForm() {
  ...
  const form = useForm<InvoiceFormData>({
    ...
    defaultValues: {
      client_id: '',
      invoice_number: '1',
      invoice_date: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 30 days from now
      case_id: '',
      client_name: '',
      address_main: '',
      address_city: '',
  ...
  const handleClientChange = async (clientId: string) => {
    ...
    if (client) {
      ...
      if (shouldFetchItems) {
        ...
        const invoicingIds = Array.isArray(invoicingField) ? invoicingField : [];
        ...
        for (const invoiceId of invoicingIds) {
          try {
            ...
            if (response.ok) {
              ...
              if (rawName.includes('pending') || rawName.includes('payment complete')) {
                continue;
              }
  ...
  const onSubmit = async (data: InvoiceFormData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Format filename
        const amount = data.part_type === 'Part E' ? parseFloat(data.awarded_amount || '0') : 150000;
        const amountStr = amount % 1000 !== 0 ? `$${(amount/1000).toFixed(1)}k` : `$${Math.floor(amount/1000)}k`;
  ...
  const completedItemFields = watchedFields.invoice_items?.reduce((acc, item) => {
    return acc + (item.name && item.date ? 1 : 0);
  }, 0) || 0;
  
  const totalItemFields = watchedFields.invoice_items?.length || 0;
  const progressPercentage = requiredFieldCount + totalItemFields > 0 
    ? ((completedBaseFields + completedItemFields) / (requiredFieldCount + totalItemFields)) * 100 
    : 0;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Professional billing and invoicing with automatic calculations</p>
          </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card variant="elevated">
          ...
          <CardContent>
            <div className="space-y-6">
              <div>
                <label className="text-sm font-medium text-foreground mb-3 block">Award Type</label>
                <div className="flex items-center space-x-6">
                  <label className="flex items-center space-x-2">
                    ...
                    />
                    <span className="text-sm font-medium">Part B ($150,000)</span>
                  </label>
                  <label className="flex items-center space-x-2">
                    <input
                      type="radio"
                      ...
                      className="text-primary focus:ring-ring"
                    />
                    <span className="text-sm font-medium">Part E (Custom Amount)</span>
                  </label>
                </div>
              </div>
                ...
                <Card key={field.id} variant="outlined" className="p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Select
                      label="Service"
                      required
                      error={form.formState.errors.invoice_items?.[index]?.name?.message}
                    ...
                    <Input
                      ...
                      label="Service Date"
                      required
                      error={form.formState.errors.invoice_items?.[index]?.date?.message}
              ...
              <div className="space-y-3">
                ...
                {progressPercentage < 100 && (
                  <div className="flex items-center justify-center text-sm text-muted-foreground max-w-md mx-auto">
                    ...
                    <span>Complete all required fields to generate your professional invoice</span>
                  </div>
                )}
              </div>
...
{loading ? (
              ...
              {progressPercentage === 100 && !loading && (
                <div className="text-xs text-muted-foreground bg-success/10 border border-success/20 rounded-lg p-3 max-w-md mx-auto">
                  ✓ Ready to generate your Excel invoice with automatic calculations
                </div>
              )}
            </div>
          </CardContent>
