...
class EE1Generator:
    ...
    def draw_pdf(
        ...
        signature_overlay.drawString(305, 645, middle_initial)
        ...
        if diagnosis_categories.get("beryllium_sensitivity", {}).get("selected"):
            ...
            if diagnosis_categories["beryllium_sensitivity"].get("date"):
                date_str = self.format_date(
                    diagnosis_categories["beryllium_sensitivity"]["date"]
                )
        ...
        if diagnosis_categories.get("chronic_beryllium_disease", {}).get("selected"):
            ...
            if diagnosis_categories["chronic_beryllium_disease"].get("date"):
                date_str = self.format_date(
                    diagnosis_categories["chronic_beryllium_disease"]["date"]
                )
        ...
        if diagnosis_categories.get("chronic_silicosis", {}).get("selected"):
            ...
            if diagnosis_categories["chronic_silicosis"].get("date"):
                date_str = self.format_date(
                    diagnosis_categories["chronic_silicosis"]["date"]
                )
        ...
        if signature_file:
            try:
                ...
                signature_image.save(
                    sig_buffer, format="JPEG", quality=95
                )  # High quality JPEG
                sig_buffer.seek(0)
                ...
                signature_overlay.drawString(
                    100, 155, f"[Signature processing failed: {str(e)}]"
                )
